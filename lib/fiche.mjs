...
// Codes de superficie Centris. Tout ce qui n'est pas en pieds carrés est ramené
// en mètres carrés. Sans cette conversion, un terrain de « 99.50 AR » (ares)
// s'affichait « 100 m² » au lieu de 9 950 m² — cent fois trop petit.
const AREA_TO_M2 = { MC: 1, AR: 100, HA: 10000, AC: 4046.8564 };
...
export function areaLabel(a){
  ...
  if(!n) return '';
  const fmt = x => Math.round(x).toLocaleString('fr-CA').replace(/ | /g,' ');
  if(u === 'PC') return `${fmt(n)} pi²`;
  const m2 = n * (AREA_TO_M2[u] ?? 1);
  // Au-delà de l'hectare, le chiffre en m² ne parle plus : on l'accompagne.
  if(m2 >= 10000) return `${fmt(m2)} m² (${(m2/10000).toFixed(1).replace('.',',')} ha)`;
  return `${fmt(m2)} m²`;
}
...
${localisation(p)}
    ...
    <aside class="jg-aside" aria-label="Contacter la courtière">
      <div class="jg-broker" data-reveal>
        ...
        <div class="jg-broker-foot"><a class="jg-link" href="mailto:julie.gauthier@remax-quebec.com" style="font-size:0.875rem">Écrire à Julie</a><img src="brand_assets/crystal-long-noir.png" alt="RE/MAX Crystal" style="height:20px;width:auto;display:block;opacity:0.9"></div>
      </div>

      <form class="jg-panel" id="jg-contact-form" action="https://formspree.io/f/xdeokzpb" method="POST" autocomplete="off">
...
(function(){
  ...
  ready(function(){
    ...
    var cf=root.querySelector('#jg-contact-form');if(cf)cf.addEventListener('submit',function(e){e.preventDefault();var s=root.querySelector('#jg-form-status');var sb=cf.querySelector('button[type="submit"]');if(sb)sb.disabled=true;if(s)s.textContent='Envoi en cours…';fetch(cf.action,{method:'POST',body:new FormData(cf),headers:{'Accept':'application/json'}}).then(function(r){if(r.ok){if(s)s.textContent='Merci, votre demande a été transmise à Julie.';cf.reset();}else{if(sb)sb.disabled=false;if(s)s.textContent='Une erreur est survenue, veuillez réessayer.';}}).catch(function(){if(sb)sb.disabled=false;if(s)s.textContent='Une erreur est survenue, veuillez réessayer.';});});
  });
})();
<\/script>`;
...
<link rel="canonical" href="https://juliegauthier.immo/nos-proprietes/${p.slug}/">
<meta property="og:url" content="https://juliegauthier.immo/nos-proprietes/${p.slug}/">
...
${ficheJs}
<script>
/* Validation en français. Les messages natifs suivent la langue du navigateur et
   non celle de la page : sur ce site fr-CA, un visiteur configuré en anglais
   voyait « Please fill out this field. ». On fournit donc nos propres textes. */
(function(){
  var PAR_TYPE = {
    valueMissing: { checkbox:"Veuillez cocher cette case pour continuer.",
                    radio:"Veuillez choisir une option.",
                    "select-one":"Veuillez choisir une option.",
                    email:"Veuillez saisir votre adresse courriel.",
                    tel:"Veuillez saisir votre numéro de téléphone." },
    typeMismatch: { email:"Veuillez saisir une adresse courriel valide, par exemple vous@exemple.ca.",
  ...
  var SIMPLE = {
    ...
    tooShort:"Ce texte est trop court.",
    tooLong:"Ce texte est trop long.",
    rangeUnderflow:"La valeur est trop petite.",
    rangeOverflow:"La valeur est trop grande.",
    stepMismatch:"Cette valeur n'est pas valide.",
    badInput:"Veuillez saisir une valeur valide."
  };
  function message(el){
    var v = el.validity;
    for (var cle in SIMPLE) {
      if (!v[cle]) continue;
      var parType = PAR_TYPE[cle];
      return (parType && parType[el.type]) || SIMPLE[cle];
    }
  ...
  // Sans remise à zéro, le champ resterait invalide même une fois corrigé.
  function reset(e){
    var el = e.target;
